...
import { orientationQuestions, orientationIndex } from './orientation-data';

type ViewState = 'index' | 'question' | 'exit';

function OrientatePage() {
  ...
  // Load last read question from localStorage on mount
  useEffect(() => {
    const lastQuestion = localStorage.getItem('fieldnodes-last-orientation');
    if (lastQuestion && parseInt(lastQuestion) >= 1 && parseInt(lastQuestion) <= 10) {
  ...
  useEffect(() => {
    if (currentQuestion) {
      localStorage.setItem('fieldnodes-last-orientation', currentQuestion.toString());
    }
  }, [currentQuestion]);

  // Load content based on view state
  useEffect(() => {
    let contentLines: string[] = [];
    
    if (viewState === 'index') {
      contentLines = orientationIndex;
    } else if (viewState === 'question' && currentQuestion) {
      const question = orientationQuestions.find(q => q.id === currentQuestion);
      if (question) {
        contentLines = question.content;
      }
    ...
    const startDelay = viewState === 'question' ? 300 : 500;
    timeoutId = setTimeout(typeNextChar, startDelay);

    return () => clearTimeout(timeoutId);
  }, [viewState, currentQuestion]);
    ...
    if (isTypingComplete && inputRef.current) {
      ...
      if (viewState === 'question') {
        const hasPrompt = typedContent.some(line =>
          (line || '').includes('return to orientation index? (y/n)')
        );
        if (hasPrompt) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (viewState === 'index' && isTypingComplete && !awaitingReturn && !showDropdown) {
      ...
      if (e.key === 'Enter' && highlightedIndex) {
        e.preventDefault();
        setCurrentQuestion(highlightedIndex);
    ...
    if (e.key === 'Escape' && (viewState === 'question' || viewState === 'exit')) {
      e.preventDefault();
      setViewState('index');
      ...
      // Handle number selection from index
      ...
      // Handle y/n response (when Enter is pressed)
      if (awaitingReturn && isTypingComplete) {
        const lower = input.trim().toLowerCase();
        if (lower === 'y' || lower === 'yes') {
          // 'y' = return to orientation index
          setViewState('index');
          setCurrentQuestion(null);
          setInput('');
          setAwaitingReturn(false);
          ...
          // 'n' = exit (go to home for questions 1-9, show exit message for question 10)
      ...
      // Handle exit view
      if (viewState === 'exit' && isTypingComplete) {
        router.push('/');
        return;
      }
  ...
  const renderLine = (line: string, idx: number) => {
    ...
    const isQuestionNumber = /^\[(\d+)\]/.test(line);
    const isPromptLine = line.includes('return to orientation index?') || 
    ...
    const isTitle = viewState === 'question' && idx === 1;
    const isExitMessage = viewState === 'exit' && (line.includes('you are now part') || line.includes('ideas move'));
    ...
    if (isQuestionNumber && viewState === 'index') {
      const match = line.match(/^\[(\d+)\]/);
      const questionId = match ? parseInt(match[1]) : 0;
      const questionText = line.replace(/^\[\d+\]\s*/, '');
      const isHighlighted = highlightedIndex === questionId;
      ...
      return (
        <div
          ...
          onMouseEnter={(e) => {
            if (isTypingComplete) {
              setHighlightedIndex(questionId);
            }
          }}
          onMouseLeave={(e) => {
            if (isTypingComplete && highlightedIndex === questionId) {
              setHighlightedIndex(null);
            }
          }}
        >
    ...
    if (isExitMessage) {
      return (
        <div key={idx} style={{ marginBottom: '8px', lineHeight: 1.6, fontSize: '14px' }}>
          <span style={{ color: tokens.color.field.lilac, fontStyle: 'italic', fontWeight: 500 }}>
            {line}
            {showCursor && <span style={{ color: tokens.color.text.primary }}>▋</span>}
          </span>
        </div>
    ...
    if (isPromptLine) {
      return (
        <div key={idx} style={{ marginBottom: '6px', lineHeight: 1.6, fontSize: '13px', marginTop: '12px' }}>
          <span style={{ color: tokens.color.text.secondary }}>
            {line}
            {showCursor && <span style={{ color: tokens.color.text.primary }}>▋</span>}
          </span>
      ...
      >
        ...
        >
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <span>field_nodes_orientation</span>
          </div>
        ...
        >
          ...
          {isTypingComplete && (
            ...
            >
              <span style={{ color: tokens.color.text.secondary, fontWeight: 500 }}>guest@fieldnodes:~FIELD</span>
